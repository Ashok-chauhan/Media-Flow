...
////////////////////////// added by ashok 08/10/2010 //////////////////////////////
                ...
                foreach ($itemElements as $element) {
		              switch ($element->nodeName) {
							 ...
							 switch ($mediaGroup->nodeName) {
								case "media:content":
									 ...
									 $length = $mediaGroup->attributes->length;
										...
										for($j=0; $j < $length; ++$j){
											if ($mediaGroup->attributes->item($j)->name == 'type')
													 $feedDef['video_type'] = $mediaGroup->attributes->item($j)->value;
										}
                                      break 2; 
									  }
									}
						...
						case "media:thumbnails":
								$length = $element->attributes->length;
									for($i = 0; $i < $length; ++$i){
										//if($element->attributes->item($i)->name =='url' && $element->attributes->item(0)->value >= 720 ){
										
											if($element->attributes->item($i)->name =='url' && $element->attributes->item(1)->value==1920){
						...
						case "media:description":
							$feedDef['description'] = $element->nodeValue;
								break;
						...
						case "media:content":  
							$mtype = array();
							 $length = $element->attributes->length;
								 for ($i = 0; $i < $length; ++$i) {
									...
									#if($element->attributes->item(2)->value == 'video/mp4'){
									if(isset($element->attributes->item(2)->name) == 'type'){
									
									if($element->attributes->item($i)->name =='bitrate' && $element->attributes->item($i)->value >0){ 
										 $feedDef['uri'] = $element->attributes->item(4)->value;
										 $feedDef['caption'] = $this->get_cc($feedDef['uri']);
										 									
									 }elseif($element->attributes->item($i)->name == 'url' && $element->attributes->item(0)->name !='bitrate'){
										$feedDef['uri'] = $element->attributes->item($i)->value;
										$feedDef['caption'] = $this->get_cc($feedDef['uri']);  
									 }
							...
							///// krdo fixs bof 25/10/16 ////
									if($element->attributes->item($i)->name=='url'){
										 if($element->attributes->item(0)->value=='image'){
											 $feedDef['icon_uri'] = $element->attributes->item($i)->value;
										 }
										 if($element->attributes->item(0)->value=='video'){
											 $feedDef['uri'] = $element->attributes->item($i)->value;
    ...
    function refreshContent($category_id = '')
    {
        ...
        foreach ($stories as $key => $story) {
            ...
            if (sizeof($storyEntry) == 0 && ($story['title'])) {
