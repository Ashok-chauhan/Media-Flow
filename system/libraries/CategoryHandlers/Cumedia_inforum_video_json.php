...
error_reporting(0);
class Cumedia_inforum_video_jsonHandler {
...
$feedContents = curl_exec($ch);
	...
	function getVideos() { 
		...
		$videos = $this->allVideos($itemElements); //Parsing all videos including jwplayer.
...
// not in use.
	...
	function allVideos($forumVideos){
		$feedDef = $this->getFeedDef();

		foreach($forumVideos->data->videoSections as $videoSections){
			foreach($videoSections->item->section->page->latest_video->items as $item){
				if (strcasecmp($item->type, 'JwPlayerVideo') == 0) {
					$count =$count+1;
					$feedDef['priority'] = $count;
					$feedDef['title'] = $item->title;
					$feedDef['pub_date'] = $item->updated;
					$feedDef['assetid'] = microtime();
					$feedDef['uri'] = trim('https://content.jwplatform.com/videos/'.$item->fields->jwplayerVideoId.'-xujEMwLT.mp4');
