...
//type=>story added for khq
    ...
    function __construct($category)
    {
		...
		//Publicaiton which do not have to share.
		$doNotShare= array(550,551,552,553,554,555,556); //added on 14.6/2019.
		// Preparing Share url for video shareing.
		$pubs = array(90,91,92,96,95,97);
		$publisher_url = $CI->getById($category['publication_id'], 'publication');
	
	/**
    if(!in_array($publisher_url['id'], $doNotShare)){

		if(in_array($publisher_url['id'], $pubs)){ 
			$this->share_uri ='http://'.parse_url($publisher_url['publisher_website'], PHP_URL_HOST).'/clip/?clipid=';
			$this->sharingTV = true; //$this->sharingTV = true; commented (false) on request to stop video sharing for calkins tv on 19/11/2014.
		}else{
		$share = (explode('/', parse_url($category['source_uri'], PHP_URL_PATH)));
		$share_channel ='channelid='.$share[5];
		//$publisher_url = $CI->getById($category['publication_id'], 'publication');
		$this->share_uri = 'http://'.parse_url($publisher_url['publisher_website'], PHP_URL_HOST).'/clip/?'.$share_channel; 
		//?TNNoMobile added on Laura's request to redirect it on desctop site instead on mobile on 18/12/2014.
		if(array_key_exists(5,$share)){
			$this->sharing = true;
		}else{
			$this->sharing = false;
		}
       ...
       // var_dump($rssRoot);
        ...
        foreach ($children as $child) {
            if ($child->nodeName == 'item') {
                ...
                foreach ($itemElements as $element) {
		              switch ($element->nodeName) {
                        ...
                        case "guid":
							...
							//Share url
							/*if($this->sharing){
							$feedDef['share_url']= trim($this->share_uri.'&mediaid='.$element->nodeValue);
							}*/
                            break;
						case "wn:clipid":
												...
												if(substr(parse_url($mediaGroup->attributes->item($i)->value, PHP_URL_PATH),-4) ==='.mp4'){
                                                    ...
                                                    $feedDef['uri'] = $mediaGroup->attributes->item($i)->value;
													 ...
													 if ($mediaGroup->attributes->item($i)->name == 'medium' && $mediaGroup->attributes->item($i)->value =='video' || $mediaGroup->attributes->item($i)->value == 'audio'){
                                                        print '<li> type '.$mediaGroup->attributes->item($i)->value;
							 ...
							 $length = $element->attributes->length;
								 for ($i = 0; $i < $length; ++$i) {
									 ...
									 if($element->attributes->item($i)->name == 'url'){
										...
										$feedDef['caption'] = $this->get_cc($feedDef['uri']);  
									 }
									
									if ($element->attributes->item($i)->name == 'url' && $element->attributes->item(1)->name == 'adaptive'){
										$feedDef['uri'] = $element->attributes->item($i)->value;
										$feedDef['caption'] = $this->get_cc($feedDef['uri']);  
										// following changed done due to 0 bitrate in client feed(blank video) on 10/3/2016.
									 }

									 if($element->attributes->item($i)->name == 'url' && $element->attributes->item(0)->value =='image/jpeg'){
										$feedDef['icon_uri'] = $element->attributes->item($i)->value;
									 ...
									 if($element->attributes->item($i)->name =='bitrate' && $element->attributes->item($i)->value >0){ 
										 $feedDef['uri'] = $element->attributes->item(4)->value;
										 $feedDef['caption'] = $this->get_cc($feedDef['uri']);
										 									
									 }
									if($element->attributes->item($i)->name == 'url' && $element->attributes->item(0)->name !='bitrate'){
										$feedDef['uri'] = $element->attributes->item($i)->value;
										$feedDef['caption'] = $this->get_cc($feedDef['uri']);  
									 }
									 *commented on 15/3/2016
									 */
									 ...
									 if ($element->attributes->item($i)->name == 'type'){
										 ...
										 if($mtype[0] =='image'){
											...
											$feedDef['video_type'] = $element->attributes->item($i)->value;

											
										 }
										 
									 }
									  if(array_key_exists(0,$mtype) && $mtype[0] == 'video'){
										  $feedDef['type'] = $mtype[0];
										  

									  }elseif(array_key_exists(0,$mtype) && $mtype[0] =='image'){
										  $feedDef['type'] = 'story';

									  }elseif($element->attributes->item($i)->name =='medium'){
										  $feedDef['type'] = $element->attributes->item($i)->value;
				...
				//getting right thumbnail/icon_uri from field59 added on 2/1/2018.
				if($this->field59IconUri($this->feed_uri, $feedDef['assetid'])){
					$feedDef['icon_uri'] = $this->field59IconUri($this->feed_uri, $feedDef['assetid']);
				}
				//Share url
				
                            if ($this->sharing) {
                               $feedDef['share_url'] = $this->share_uri . '&mediaid=' . $feedDef['assetid'].'&vid_url='.$feedDef['icon_uri'].'&vid_title='.urlencode($feedDef['title']).'&vid_desc='.urlencode(substr($feedDef['description'],0,60));
                              }elseif($this->sharingTV){
								   $feedDef['share_url'] = $this->share_uri.$clipId.'&vid_url='.$feedDef['icon_uri'].'&vid_title='.urlencode($feedDef['title']).'&vid_desc='.urlencode(substr(strip_tags($feedDef['description']),0,60)).'&TNNoMobile';

								  // &TNNoMobile are added for calkins tv app on 19/12/2014 on Laura's request.
							  }
...
/////// feed_order_zero is NOT USING  07/07/2012 ///////////////////
	...
	//daynamicaly creating thumbnail (icon_uri) from field59 added on 2/1/2018
	function field59IconUri($field59Url, $videoKey){
		if('player.field59.com' === parse_url($field59Url, PHP_URL_HOST)){
			return trim('https://redirect.field59.com/video/thumb/'.$videoKey.'/w600/h337.jpg');
		}
	}
