            ...
            if ($child->nodeName == 'item') {
                ...
                foreach ($itemElements as $element) {
						 ...
						 foreach ($element->childNodes as $mediaGroup) {
							 switch ($mediaGroup->nodeName) {
								case "media:content":
									 $length = $mediaGroup->attributes->length;
										...
										for ($i = 0; $i < $length; ++$i) {
											if ($mediaGroup->attributes->item($i)->name == 'url')
												if(substr(parse_url($mediaGroup->attributes->item($i)->value, PHP_URL_PATH),-5) ==='.m3u8')
                                        ...
                                        for ($i = 0; $i < $length; ++$i) {
                                            if ($mediaGroup->attributes->item($i)->name == 'url')
                                                if(substr(parse_url($mediaGroup->attributes->item($i)->value, PHP_URL_PATH),-4) ==='.mp4' && $feedDef['uri'] =='')
                                                    $feedDef['uri'] = $mediaGroup->attributes->item($i)->value;
                                    ...
                                    for($i = 0; $i < $length; ++$i){
                                        if($mediaGroup->attributes->item($i)->name =='url')
                                        $feedDef['icon_uri'] = urldecode($mediaGroup->attributes->item($i)->value);
				 ...
				 foreach ($element->childNodes as $mediaElement) {
							...
							//////////////////////////////////MULTIMEDIA MANIPULATION 12/11/2019
                        case "multimedia":
                                    ...
                                    foreach ($mediaElements as $element) {
                                        ...
                                        switch ($element->nodeName) {
                                            ...
                                            case "link":
												...
												$ext = pathinfo($element->nodeValue);
												 if($ext['extension']=='mp4'){
													$feedDef['video_type'] = 'video/mp4';
												 }else{
													 $feedDef['video_type']= 'application/octet-stream';
...
///////////////////// EOF MULTIMEDIA  MANIPULATIN //////////////
    ...
    function refreshContent($category_id = '')
    {
		...
		if(array_key_exists (substr(parse_url($story['uri'], PHP_URL_PATH),-4), $videoDef)){
