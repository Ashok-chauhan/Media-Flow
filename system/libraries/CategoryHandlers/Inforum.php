    ...
    function getStoryLink() {
        ...
        if (isset($itemElements->data->latestInSection->items)) {
            foreach ($itemElements->data->latestInSection->items as $element) {
                //print_r($element->appdataUrl);
               // print '<li> '.$element->appdataUrl;
                if(array_key_exists('appdataUrl', $element)){
        ...
        if (isset($itemElements->data->context->rootGroup->featured)) {
            foreach ($itemElements->data->context->rootGroup->featured as $featured) {
                //print_r($featured);
                if (array_key_exists('appdataUrl', $featured)) {
...
$urlArray[]='https://api.forum.cue.cloud/inforum/business/4887586-Fargodome-hosting-tech-focused-Dakota-Strike-career-expo-on-April-8/appdata';
                            ...
                            foreach ($value as $value) {
                                ...
                                if ($value->text && $href != '') {
                                    $text .= '<li><' . $anchor . ' href ="' . $href . '" target="' . $attr . '">' . $value->text . '</' . $anchor . '>';
                                     ...
                                     print '<li> related text '.$text.' --'.$value->text;
                                    //$text .= $value->text;
                                }
                            }
                        }
